...
\section{Phát hiện theo chữ ký và IOC}
Phát hiện theo chữ ký (Signature-based Detection) so khớp lưu lượng với các mẫu đã biết. Mẫu có thể là chuỗi trong payload, biểu thức chính quy, trường HTTP, truy vấn DNS, User-Agent, domain trong blacklist/IOC hoặc tổ hợp điều kiện trong header gói tin. Cách tiếp cận này phù hợp để nhận diện các kỹ thuật tấn công đã biết như SQL Injection, công cụ quét tự động, payload có cấu trúc đặc trưng hoặc tên miền độc hại đã được ghi nhận.
...
DNS có thể bị lợi dụng để truy vấn tên miền độc hại hoặc truyền dữ liệu ra ngoài thông qua các nhãn dài, khó đọc, entropy cao. Trong đồ án, truy vấn khớp domain mẫu như \texttt{chatgpt.com}, \texttt{gemini.google.com} hoặc \texttt{claude.ai} được triển khai bằng luật chữ ký DNS mặc định trong signature engine, không nằm trong nhóm luật sliding-window. Ngược lại, DNS tunneling được nhận diện bằng dấu hiệu bất thường của chuỗi truy vấn trong cửa sổ thời gian.
...
\begin{itemize}
    ...
    \item Nếu không xử lý IP fragmentation, TCP segmentation và out-of-order packets, hệ thống có thể bỏ sót hoặc phân tích sai một số luồng dữ liệu.
    \item Cảnh báo hành vi phụ thuộc vào ngưỡng và môi trường mạng cụ thể.
    \item Các kỹ thuật encode, obfuscate hoặc chia nhỏ payload có thể né tránh signature matching đơn giản.

...
Các luật hành vi và anomaly được triển khai bằng cửa sổ trượt theo thời gian. Hệ thống lưu các sự kiện gần nhất theo IP nguồn và loại bỏ dữ liệu đã nằm ngoài khung thời gian cấu hình. Riêng RULE-004 được biên dịch thành luật chữ ký DNS mặc định và chạy trong signature engine, không nằm trong sliding-window rules. Các ngưỡng trong bảng là giá trị lab/demo, chưa phải khuyến nghị production.
...
\begin{table}[H]
    ...
    \begin{tabular}{|>{\raggedright\arraybackslash}p{1.5cm}|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{4.9cm}|>{\raggedright\arraybackslash}p{1.3cm}|}
        ...
        RULE-004 & Suspicious DNS Query & Signature-based & DNS query khớp domain blacklist như \texttt{chatgpt.com}, \texttt{gemini.google.com}, \texttt{claude.ai}. & Medium \\ \hline
        RULE-005 & DNS Tunneling & \makecell[l]{Behavior/\\anomaly-based} & $\geq$ 5 DNS query đáng ngờ từ cùng source trong 30 giây. & Medium \\ \hline
    \end{tabular}
...
Ngoài luật hành vi, hệ thống hỗ trợ một tập con cú pháp kiểu Suricata/Snort. Người dùng có thể truyền tệp luật qua tham số \texttt{--rules}. Trong demo, tệp \texttt{custom.rules} chứa ba luật HTTP:
...
Subset prototype hỗ trợ các thành phần cơ bản như header luật, \texttt{content}, \texttt{pcre}, \texttt{nocase}, một số sticky buffer HTTP/DNS và threshold đơn giản. Các tính năng chưa hỗ trợ đầy đủ gồm \texttt{depth/offset}, \texttt{distance/within}, \texttt{byte\_test}, \texttt{flowbits}, TCP stream inspection, \texttt{file\_data} và tối ưu \texttt{fast\_pattern}. Tùy chọn \texttt{flow} hiện chỉ được kiểm tra ở mức hướng cơ bản, chưa phải stateful TCP flow tracking như IDS production.
